...
import { sentDocuments } from '../data/categorizedData';
...
export function Sent() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">Sent Documents</h1>
        <p className="mt-1 text-slate-600">Documents you have sent to others</p>
      </div>
        ...
        <div className="px-6 py-4 border-b border-slate-200">
          <div className="flex items-center justify-between">
            <p className="text-sm text-slate-600">
              You have sent <span className="font-medium text-slate-900">{sentDocuments.length}</span> documents
            </p>
          </div>
        </div>
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {sentDocuments.map((doc) => (
                <tr key={doc.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm text-slate-600">{formatDate(doc.createdAt)}</span>
                  </td>
